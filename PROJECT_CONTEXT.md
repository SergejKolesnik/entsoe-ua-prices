...
- Operator conflicts can be inspected with the additive `diagnose-operator-conflict` command or the manual `diagnose_operator` workflow task. It compares stored and currently published field identities read-only and reports only Kyiv hours and differing field names; it never rewrites prices or persists the diagnostic download.
...
- Production physical-flow history can be extended through the manual GitHub `backfill_flows` task. Each run is bounded to 14 inclusive days, collects selected borders in both directions, uses repository secrets, and can safely retry transient ENTSO-E failures.
...
- The Overview tab includes a deterministic daily RDN commentary for the selected delivery date. It requires complete RDN price days, reuses only locally stored validated context, and labels each secondary source as confirmed or unavailable. It is descriptive rather than causal and does not write a daily text record to Neon; a later delivery-channel decision can add notifications without changing its calculation contract.
- The Overview daily card presents only aligned RDN data: price movement, seven-day position, and RDN volume. Neighboring markets and physical flows are excluded from this card because their data can lag the selected RDN delivery day; their dedicated analytical tabs retain the full dated coverage. A six-region Open-Meteo weather block is selected strictly as-of the start of the delivery day, so a later forecast revision cannot appear in a historical card. It is unweighted and descriptive until demand and generation coverage can support a calibrated market signal.
...
- Feature `codex/gas-2023-monthly-history` adds the user-verified annual 2023
  ...
  and monthly consumption chart/table. A pending follow-up uses the new explicit
  commodity-price-without-VAT column as a distinct 2023 price series; it does not
  infer a VAT-exclusive full price or delivery components. Local tests and Streamlit AppTest against
  ...
  preserved. The 2023 consumption UI is published; a separate release approval is
  required for the pending price-series follow-up. See
