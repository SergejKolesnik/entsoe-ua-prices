...
def get_ua_prices():
    ...
    try:
        ...
        if r.status_code != 200 or '<Point>' not in r.text:
            print("На завтра ще немає, перевіряємо сьогодні...")
        ...
        if r.status_code == 200 and '<Point>' in r.text:
            ...
            with open(file_name, 'a', newline='') as f:
                ...
                for p in points:
                    ...
                    writer.writerow([target_date.date(), f"{int(pos)-1:02d}:00", val, curr_text])
            
            print(f"Успішно додано {len(points)} записів за {target_date.date()} ({curr_text})")
        else:
            print("Дані на платформі поки що недоступні для запиту.")
