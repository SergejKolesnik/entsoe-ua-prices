...
Scheduled Operator failures are stored as sanitized categories. Known immutable-price conflicts are reported as `ValueError:price_conflict`; invalid artifacts and other validation failures use separate categories. Raw exception messages, source payloads, prices, and connection details are never written to the monitoring journal.
